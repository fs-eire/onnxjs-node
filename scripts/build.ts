...
const DEBUG = process.argv.slice(2).indexOf('--debug') !== -1 ||
    process.argv.slice(2).indexOf('-d') !== -1;
...
const cmakejs =
    spawnSync(CMAKE_JS_FULL_PATH, cmakejsArgs, {shell: true, stdio: 'inherit'});
if (cmakejs.status !== 0) {
  if (cmakejs.error) {
...
if (os.platform() === 'win32') {
  ...
  fs.copyFileSync(
      path.join(BUILD_OUTPUT_FOLDER, 'onnxruntime.node'),
      path.join(BIN, 'onnxruntime.node'));

  // copy pdb file (this is useful in DEBUG mode)
  if (DEBUG &&
      fs.existsSync(path.join(BUILD_OUTPUT_FOLDER, 'onnxruntime.pdb'))) {
    fs.copyFileSync(
        path.join(BUILD_OUTPUT_FOLDER, 'onnxruntime.pdb'),
        path.join(BIN, 'onnxruntime.pdb'));
  }

  // copy DLL
  fs.copyFileSync(
      path.join(ONNXRUNTIME_DIST, 'bin/win-x64/native/mkldnn.dll'),
      path.join(BIN, 'mkldnn.dll'));
  fs.copyFileSync(
      path.join(ONNXRUNTIME_DIST, 'bin/win-x64/native/onnxruntime.dll'),
      path.join(BIN, 'onnxruntime.dll'));
